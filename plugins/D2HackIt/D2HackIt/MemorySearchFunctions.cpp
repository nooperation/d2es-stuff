...
DWORD PRIVATE GetMemoryAddressFromPattern(LPSTR szDllName, LPCSTR szSearchPattern, DWORD offset)
{
	...
	if(szSearchPattern[0] == '!')
	{
		...
		if(hModule) lResult = (DWORD)GetProcAddress(hModule, &szSearchPattern[1]);

		// No luck, lets try by ordinal number instead
		if(!lResult)
		{
			lResult = (DWORD)GetProcAddress(hModule, (LPCSTR)MAKELONG(strtoul(&szSearchPattern[1], &t, 10), 0));
		}
	...
	if(szSearchPattern[0] == '@')
	{
		return SearchAddress + strtoul(&szSearchPattern[1], &t, 0x10);
	} 
	...
	lResult = (DWORD)PatternSearch((BYTE*)SearchAddress, SearchSize, pPattern, len);
	delete [] pPattern;
