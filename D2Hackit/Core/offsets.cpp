...
#pragma region ThisIsWhyWeCantHaveNiceThings
	// Just copy-paste D2FUNCPTR, D2VARPTR, D2ASMPTR functions from d2ptrs.h here and this will link
	//  the function pointers CORRECTLY. The previous implementation relied on the compiler to generate
	//  all of the function pointers sequentially, 4 bytes apart, which the compiler used in 2012 doesn't.
	//  this just expands what the previous code use to do.

	#pragma push_macro("D2FUNCPTR")
	#pragma push_macro("D2VARPTR")
	#pragma push_macro("D2ASMPTR")

	#undef D2FUNCPTR
	#undef D2VARPTR
	#undef D2ASMPTR
	#define D2FUNCPTR(d1,v1,t1,t2,o1) *((DWORD *)&d1##_##v1) = GetDllOffset(*((DWORD *)&d1##_##v1));
	#define D2VARPTR(d1,v1,t1,o1)     *((DWORD *)&p_##d1##_##v1) = GetDllOffset(*((DWORD *)&p_##d1##_##v1));
	#define D2ASMPTR(d1,v1,o1)        *((DWORD *)&d1##_##v1) = GetDllOffset(*((DWORD *)&d1##_##v1));
