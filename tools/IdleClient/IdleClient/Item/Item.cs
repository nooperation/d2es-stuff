...
namespace CharacterEditor
{
	public class Item : INotifyPropertyChanged
	{
		...
		private bool IsPacketData = true;
		...
		public byte Unknown0b
		{
			...
			set { SetData("Unknown0b", value); }
		}
		public byte PacketType
		{
			get { return (byte)GetDataValue("PacketType"); }
			set { SetData("PacketType", value); }
		}
		public uint ItemID
		{
		...
		private void ReadItemData()
		{
			...
			//return;
			if (IsSimpleItem || IsGamble || ItemCode == "tbk" || ItemCode == "ibk" || ItemCode == "box")
			{
				return;
			}
		...
		private void ReadItemDataSimple()
		{
			if (IsPacketData)
			{
				ReadData("MessageID", 8); //message
				ReadData("Action", 8);
				ReadData("Unknown0b", 8); //message size
				ReadData("PacketType", 8);
				ReadData("ItemID", 32);
			}
		...
		private static void CreateDataIndicies()
		{
			...
			// 0x9C Packet data
			dataIndicies.Add("MessageID", index++);
			dataIndicies.Add("Action", index++);
